...
export async function analyzeCompanyEnrichment(admin, anthropic, companyId) {
  ...
  const stream = anthropic.messages.stream({
    model: MODEL,
    // 2026-07-30: bumped from 6000 after a real company (normal-sized input -
    // 6000-char website text, 5 reviews, nothing unusual) hit the ceiling and
    // was correctly rejected rather than silently truncated. Real headroom,
    // not a cost increase - only spent if the model actually needs it.
    max_tokens: 8000,
