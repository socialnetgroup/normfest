 ...
 * opportunity's catalog_category + search_terms, ranked by real customer
 * interest. Primary signal (added 2026-08-22): `products.access_count_2026`
 * - real webshop page-access counts from Normfest's own "Zugriffe je Artikel
 * 2026" export (§14 item 134's promised, now-delivered stronger signal),
 * imported via scripts/import-access-counts.mjs. Anis's own original example
 * (a tire shop's opportunity surfacing valve caps but not balance weights,
 * which genuinely sell more) is exactly the class of miss this targets -
 * real customer traffic, not a proxy. Only ~43% of products have a real
 * access count (the export doesn't cover the whole catalog), so candidates
 * without one fall back to the original proxy - how often a candidate
 * appears as the TARGET of a real cross-sell relation
 * (product_relations.related_product_id, i.e. how often the webshop's own
 * merchandising, §13 M4's 141k-row crawl, considered it worth recommending
 * alongside something else) - as a tiebreak, never silently dropped to the
 * bottom by an absent access count. */
async function rankCandidates(admin, candidates) {
  ...
  const ranked = [...candidates].sort((a, b) => {
    const accessA = a.access_count_2026 ?? -1;
    const accessB = b.access_count_2026 ?? -1;
    if (accessA !== accessB) return accessB - accessA;
    return (importanceById.get(b.id) ?? 0) - (importanceById.get(a.id) ?? 0);
  });
  ...
  // documented for DIN- & Normteile elsewhere in this app): a same-name
  // pack-size/dimension variant is never a genuinely different product from
  // the agent's point of view, so it must never occupy its own slot.
  // Originally padded remaining slots with these duplicates once the real
  // distinct-name pool ran out - Anis, 2026-08-22, after seeing a real
  // 6-slot Reifenmontage list where 3 of the 6 were just size variants of
  // one "Gummiventile für PKW und Motorrad" name: "damit dies keine slots
  // nimmt" - better to show fewer real distinct products than pad with
  // near-duplicates. Now keeps only the first (highest-ranked, per the
  // access-count/cross-sell sort above) product per distinct name.
  ...
  return distinct.slice(0, MATCHED_PRODUCTS_LIMIT);
}
...
export async function matchCatalogProducts(admin, catalogCategory, searchTerms) {
  ...
  const { data: scoped, error } = await admin
    .from("products")
    .select("id, sku, name, category_name, access_count_2026")
