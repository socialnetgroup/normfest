...
// Soft tier (2026-08-09), Anis: "do the soft match you suggested to clean
// up a bit, since 1k too much by hand" - after the decisive tier above
// cleared 244/1,291, the remaining 1,047 is still too much manual work.
// Looser on purpose but still real: a HIGH absolute score (the candidate's
// name genuinely resembles the company, not just "better than the other
// options") even without a strict 2x margin over the runner-up. A high
// absolute score is real signal on its own - two candidates can both
// plausibly relate to the company name without either being a wrong-
// business false positive the way a low-score "best of a bad set" pick
// would be.
export const NAME_MATCH_SOFT_THRESHOLD = 0.7;
export const NAME_MATCH_SOFT_MARGIN = 1.3;
...
/** Decisive-or-soft name match against a company name, or null if neither tier fires. */
export function bestNameMatch(places, companyName) {
  ...
  if (decisive) return top.place;

  const soft =
    top.score >= NAME_MATCH_SOFT_THRESHOLD &&
    (runnerUpScore === 0 || top.score >= runnerUpScore * NAME_MATCH_SOFT_MARGIN);
  return soft ? top.place : null;
}

// Anis (2026-08-11): "where the name match ist 100% merge both, tripple
// results and zusammenführen" - a real gap in bestNameMatch() above: it
// only ever compares the TOP candidate against a single runner-up margin,
// so when 2 (or 3+) candidates ALL score a perfect 1.0 name match - a tie
// at the top, not "one clearly better than the rest" - neither the
// decisive nor soft tier fires (margin 1.0/1.0 = 1x, under both 2x and
// 1.3x thresholds) and the company falls through to the manual queue even
// though a perfect word-for-word name match is about as strong a "same
// real business, multiple listings" signal as the exact-address merge
// above. Handled as its own tier, checked before the single-pick tiers:
// every candidate scoring exactly 1.0 gets merged together (mergeCandidates,
// same real-quotes-kept-separate shape as the address-based merge), not
// arbitrarily reduced to one.
export function perfectNameMatchGroup(places, companyName) {
  if (!companyName || places.length < 2) return null;
  // A "perfect" (1.0) score on just ONE surviving distinguishing word is a
  // real false-positive risk, not genuine confidence - e.g. "Automobile
  // A&N" vs "A&D Automobile" both collapse to the single token
  // {automobile} once the length>1 filter strips the single-letter "A&N"/
  // "A&D" parts, scoring a meaningless perfect match between two likely-
  // different companies. Found by spot-checking a real dry-run before
  // trusting this tier at scale, per Anis's "confidently merge" ask -
  // confidence requires the match to be on ≥2 real words, not one.
  if (normalizeCompanyName(companyName).size < 2) return null;
  const perfect = rankByNameMatch(places, companyName)
    .filter((r) => r.score === 1)
    .map((r) => r.place);
  return perfect.length >= 2 ? perfect : null;
}
...
export function pickResolution(places, company) {
  ...
  const perfectGroup = perfectNameMatchGroup(places, company.name);
  if (perfectGroup) return { status: "resolved_merged", places: perfectGroup };
  ...
  // a loose text search, not duplicate listings - a decisive (or, failing
  // that, a soft) name match against the real company name resolves a
  // real share of those automatically without ever hitting the manual
  // queue.
  const nameMatch = bestNameMatch(places, company.name);
