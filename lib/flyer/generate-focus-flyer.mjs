...
// end." 100% real, unaltered product photos - only their on-page
// arrangement is synthetic (rotation/offset/shadow), never the pixels.
function drawImageGroup(ctx, images, boxX, boxY, boxW, boxH) {
  ...
  const n = list.length;
  const baseSize = Math.min(boxW / (n * 0.58), boxH * 0.85);
  ...
  for (const i of order) {
    ...
    const cx = boxX + boxW / 2 + offset * baseSize * 0.4;
    const cy = boxY + boxH / 2 + Math.abs(offset) * baseSize * 0.05;
    ctx.save();
    ctx.translate(cx, cy);
    ctx.rotate(offset * 0.12);
    ctx.shadowColor = "rgba(20,24,32,0.22)";
    ctx.shadowBlur = 4;
    ctx.shadowOffsetY = 2;
...
async function drawProductCard(ctx, x, y, cellW, cardH, cellRows, imgBox, hasDesc, imageCache, storage, measureOnly) {
  ...
  if (!measureOnly) {
    ...
    for (const p of imagePaths.slice(0, 5)) {
      images.push(await getFlyerImage(p, storage, imageCache));
    }
