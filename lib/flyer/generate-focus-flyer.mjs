...
async function drawCoverPage(ctx, list, heroImageBuffer) {
  ...
  if (list.validityLabel) {
    ...
    const w = Math.min(ctx.measureText(list.validityLabel).width + 34, textPanelW);
    ctx.fillStyle = BLUE_BADGE;
    ctx.beginPath();
    ctx.roundRect(MARGIN, badgeY, w, 30, 15);
    ctx.fill();
    ctx.fillStyle = "#ffffff";
    ctx.fillText(list.validityLabel, MARGIN + 17, badgeY + 20);
    ty = badgeY + 30;
  }

  // Stat row - icon + separated pill cards (2026-08-09, Anis: "add symbols
  // to Aktionsprodukte and Kategorien... separate it" - was two bare number/
  // label pairs floating side by side with no visual structure). Counts
  // stay real/dynamic (list.productCount/categoryCount computed from the
  // list's actual rows at generation time, see generateFocusListFlyer) -
  // only the presentation changed here.
  const statY = ty + 26;
  const pillH = 42;
  const pillGap = 10;
  const pillW = (textPanelW - pillGap) / 2;
  drawStatPill(ctx, MARGIN, statY, pillW, pillH, drawTagIcon, String(list.productCount), "AKTIONSPRODUKTE");
  drawStatPill(
    ctx,
    MARGIN + pillW + pillGap,
    statY,
    pillW,
    pillH,
    drawGridIcon,
...
function drawStatPill(ctx, x, y, w, h, iconFn, value, label) {
  ...
  iconFn(ctx, x + 20, y + h / 2);

  const textX = x + 38;
  ctx.textAlign = "left";
  ctx.font = "20px Poppins Bold";
  ctx.fillStyle = "#ffffff";
  ctx.fillText(value, textX, y + h / 2 - 1);
  ctx.font = "8px Poppins SemiBold";
  ctx.fillStyle = "#cfd8e8";
  ctx.fillText(label, textX, y + h / 2 + 13);
}
...
// Variant-family cards span the full row width instead of one grid cell
// (2026-08-09, Anis: "when doing such aggregated grouped [cards], they need
// to take more place... mix it up so the whole flyer doesn't look boring in
// only one layout raster") - a fixed, generous image-group box regardless of
// the surrounding category's compact/detailed grid, since the row now has
// the full page width to work with either way.
const FAMILY_IMG_BOX = 120;
const CELL_GAP = 6;
...
export async function generateFocusListFlyer(supabase, focusListId) {
  ...
  for (let ci = 0; ci < categories.length; ci++) {
    ...
    // a fresh page) - accounts for the first row possibly being a wide
    // family row, which is taller than a normal grid row.
    const firstRowMinH = (cells[0]?.length > 1 ? FAMILY_IMG_BOX : imgBox) + 16;
    ...
    // Row packing: a variant-family cell (cellRows.length > 1) always takes
    // its own full-width row instead of sharing a row with other cards -
    // this is what actually produces the visual variety (some rows wide
    // "feature" cards, others the normal N-column grid) rather than one
    // flat repeating raster down the whole page.
    let i = 0;
    while (i < cells.length) {
      const isFamilyRow = cells[i].length > 1;
      let rowItems, rowCellW, rowImgBox;
      if (isFamilyRow) {
        rowItems = [cells[i]];
        rowCellW = PAGE_W - MARGIN * 2;
        rowImgBox = FAMILY_IMG_BOX;
        i += 1;
      } else {
        rowItems = [];
        while (rowItems.length < cols && i < cells.length && cells[i].length === 1) {
      ...
      let rowH = rowImgBox + 16;
      for (const cellRows of rowItems) {
