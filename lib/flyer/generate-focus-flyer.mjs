...
function drawCoverPage(ctx, list) {
...
// (via an offscreen canvas) and re-encode as JPEG once per unique image,
// cached across the whole flyer (many focus-list rows share the same
// representative photo).
const TARGET_IMG_PX = 220;
...
async function getFlyerImage(imagePath, storage, imageCache) {
  ...
  try {
    ...
    const smallImg = await loadImage(small.toBuffer("image/jpeg", 0.82));
