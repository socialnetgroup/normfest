...
async function drawCoverPage(ctx, list, heroImageBuffer) {
  ...
  // Validity badge - font/box bumped to match the rest of the now-larger
  // cover text (2026-08-09, Anis: "Gültig bis tile didn't follow resizing
  // of the text" - it had stayed at its original small size while the
  // headline/tagline around it grew).
  if (list.validityLabel) {
    const badgeY = ty + 18;
    ctx.font = "14px Poppins SemiBold";
    const w = Math.min(ctx.measureText(list.validityLabel).width + 40, textPanelW);
    ctx.fillStyle = BLUE_BADGE;
    ctx.beginPath();
