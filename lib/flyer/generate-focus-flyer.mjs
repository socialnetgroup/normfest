...
// text-RIGHT (not the previous image-top/text-below stacked card), a real
// product photo used as the cover's background (cropped from the reference
// mockup itself, at Anis's explicit go-ahead to use that file), and a
...
const COVER_BG_PATH = join(process.cwd(), "assets/flyer/cover-bg.png");
...
// Cover (2026-08-09) - background photo cropped directly from Anis's own
// reference mockup (explicit go-ahead to use that specific file), real
// product/list data drawn as vector text on top per the reference's layout:
// wordmark + validity pill + a small stat row + tagline.
async function drawCoverPage(ctx, list) {
  let bg = null;
  try {
