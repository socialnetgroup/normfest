...
// Product card (2026-08-09, 4th pass) - image LEFT / text stacked RIGHT.
// One function draws AND measures: called with measureOnly=true it runs the
// exact same font-setting/wrapText/increment logic used for real drawing
// but skips every paint call, so the returned content height can never
// drift from what actually gets drawn. Used to size each ROW's height from
// its own real content before committing to a shared card height for that
// row, instead of one fixed height for every card in the flyer. `cellRows`
// is one product (length 1) or a same-photo variant family (length > 1,
// see groupByImage) - a variant family shares one photo and one name/
// description, with each variant's own SKU/price/note listed compactly.
async function drawProductCard(ctx, x, y, cellW, cardH, cellRows, imgBox, hasDesc, imageCache, storage, measureOnly) {
  ...
  if (!measureOnly) {
    ...
    if (primary.image_path) {
      const img = await getFlyerImage(primary.image_path, storage, imageCache);
      if (img) {
  ...
  const nameLines = wrapText(ctx, primary.name, textW).slice(0, hasDesc ? 3 : 2);
...
export async function generateFocusListFlyer(supabase, focusListId) {
  ...
  for (let ci = 0; ci < categories.length; ci++) {
    ...
    const cells = groupByImage(categoryRows);
    ...
    const minRowH = imgBox + 16;

    if (ctx === null) {
      startPage(category, number, false);
    } else if (cursorY + minRowH > CONTENT_BOTTOM) {
      finishPage(false);
      startPage(category, number, false);
    } else {
    ...
    for (let i = 0; i < cells.length; i += cols) {
      const rowItems = cells.slice(i, i + cols);
      ...
      for (const cellRows of rowItems) {
        const h = await drawProductCard(ctx, 0, 0, cellW, 0, cellRows, imgBox, hasDesc, imageCache, supabase.storage, true);
        rowH = Math.max(rowH, h);
      }
      rowH += CELL_GAP;

      const isLastRow = isLastCategory && i + cols >= cells.length;
      ...
      for (let col = 0; col < rowItems.length; col++) {
        ...
        await drawProductCard(ctx, x, cursorY, cellW, rowH, rowItems[col], imgBox, hasDesc, imageCache, supabase.storage, false);
      }
      cursorY += rowH;
    }
  }
