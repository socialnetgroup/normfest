...
export const TOOLS = [
  ...
  },
  {
    name: "search_kb",
    description:
      "Durchsucht Skript (Gesprächsleitfaden, 5-Phasen-Struktur, Formulierungen) und Wissen (Hintergrundwissen) per Volltextsuche (Wortsuche ohne Stemming — Suchbegriffe müssen in der Wortform vorkommen, die im Text steht). WICHTIG: Der Inhalt der Sammlung 'skript' ist auf Bosnisch/Kroatisch geschrieben, 'wissen' auf Deutsch — formuliere query in der jeweiligen Sprache der Sammlung, nicht in der Sprache der Agent-Frage. Wenn eine Suche nichts findet, versuche höchstens 1-2 andere Suchbegriffe (auch übersetzt) — nicht endlos wiederholen.",
    input_schema: {
      type: "object",
      properties: {
        query: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Schlägt vor, ein Anrufergebnis zu erfassen. outcome-Werte: 'sold' (verkauft), 'rejected' (Abgelehnt — echten Ansprechpartner erreicht, kein Verkauf; objection z.B. 'Kein Interesse'/'Zu teuer'/'Genug Vorrat'), 'not_relevant' (Nicht angetroffen — niemand hat sich gemeldet; objection z.B. 'Keine Verbindung'/'Durchgeklingelt'/'Anrufbeantworter'), 'keine_zeit' (Gespräch zustande gekommen, aber nicht mit dem echten Ansprechpartner), 'nicht_besucht' (Firma heute gar nicht kontaktiert — comment ist hier PFLICHT und muss erklären warum). WICHTIG: Dieser Aufruf führt NICHTS aus — er zeigt dem Agenten nur eine Bestätigungskarte im Chat. Sag dem Agenten, dass er die Karte bestätigen muss; behaupte NIE, dass das Feedback schon gespeichert wurde.",
    input_schema: {
      type: "object",
      properties: {
        company_id: { type: "string" },
        outcome: { type: "string", enum: ["sold", "rejected", "not_relevant", "keine_zeit", "nicht_besucht"] },
 ...
 */
export async function runChatTurn({ anthropic, model, supabase, isAdmin, companyContext, messages, onText, maxTurns = 8 }) {
  let assistantText = "";
  ...
  for (let turn = 0; turn < maxTurns; turn++) {
    ...
    // Latency fix (2026-08-08, Alan's pilot feedback: "chat funkcija malo
    // spora"): when the model legitimately calls more than one tool in the
    // same turn (e.g. search_kb + get_objection_cards), they were previously
    // awaited one at a time in a for-loop even though they're independent
    // Supabase RPCs — pure serialized network latency for no reason.
    // Promise.all preserves array order, so tool_result[i] still matches
    // toolUseBlocks[i]'s tool_use_id. Confirm-only tools stay synchronous
    // (no I/O, just building the pending_action payload).
