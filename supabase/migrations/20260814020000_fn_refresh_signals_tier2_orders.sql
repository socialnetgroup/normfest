-- Wires the new orders/order_items (§4.4, migration 20260814010000) into
-- fn_refresh_signals() - Anis: "Wire orders into Tier-2 signals anyway"
-- (2026-08-14), explicitly choosing to do this now even though volume is
-- still just 46 invoices from one mailbox dump.
--
-- Real data checked directly before writing any thresholds (not guessed):
-- 26 companies have real orders, spread very thin (most have exactly 1).
-- Only a few real signals are actually supportable right now:
--   - dormant_winback: 1 real candidate (Konrad Fahrzeug Zentrum, last
--     order 465 days ago).
--   - replenishment_due: 4 companies have >=3 clean (non-needs_review)
--     orders with a real computable cycle; applying the same
--     avg_gap * 1.25 overdue math already proven for feedback_replenishment
--     (§6.2b) finds 2 real overdue candidates (Platinium Auto Service GmbH,
--     Florian Ludwig Transporte).
--   - declining_volume: written correctly (rolling 90d vs prior 90d), but
--     real data doesn't currently have any company with orders in BOTH
--     windows, so this is expected to return 0 rows today - same "don't
--     fire without data" pattern as new_product_match sitting at 0 until
--     products.launched_at was real, not a bug.
-- Deliberately NOT built this pass, and why:
--   - first_order_followup: needs a trustworthy notion of "this was their
--     first real order," which a one-time scattered historical backfill
--     can't honestly provide - every company's "first" imported invoice
--     might just be the first one an agent happened to forward, not their
--     actual first Normfest order. Revisit once order import is a genuine
--     ongoing feed, not a backfill.
--   - basket_expansion (avg order value vs peer median): with ~1 order per
--     company on average, any branche/cluster peer group is too small
--     (often n=1) for a median comparison to mean anything - would be
--     statistically fake precision, not a real signal.
--
-- Also populates the long-dormant company_rfm table (§4.5, schema existed,
-- never written to) from real orders - descriptive only, no UI reads it
-- yet, but it's now real data instead of an empty table for whenever that
-- UI gets built.
...
begin
  ...
  -- cross_sell: same dedup logic as before, capped to the top 15 rows per
  -- company by score (see 20260811040000's header) - the real fix for the
  ...
  -- ── Tier 2: real orders/order_items (new, 2026-08-14) ──────────────────
  -- needs_review orders are excluded everywhere below - a flagged invoice's
  -- numbers might be wrong, so it can't be trusted to compute a real cycle
  -- or revenue window off of.

  delete from company_rfm;
