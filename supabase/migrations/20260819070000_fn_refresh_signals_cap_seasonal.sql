-- Real over-generation bug found (2026-08-19) while investigating Anis's
-- "no seasonal signals for agents" report: products.season windows were
-- extended to cover August (see the same-day products.season update -
-- AC-service category was 3,4,5,6,7, missing the actual peak-heat month),
-- which correctly brought seasonal_push back from 0 rows - but checking the
-- real resulting volume found every one of 1,742 matched companies got ALL
-- 63 AC-service products as separate seasonal_push signals (avg = max = 63
-- rows/company, 109,744 rows total). Same root shape as the cross_sell
-- volume problem fixed in 20260811040000 - the query fires one signal row
-- per matching PRODUCT in an affinity-matched category, with no cap on how
-- many products from one category get surfaced per company. Applies the
-- exact same row_number()-over-company cap pattern used there, to
-- seasonal_push and new_product_match alike (same shared _category_affinity
-- source, same shape of query).
...
begin
  ...
  -- capped to the top 5 products per company by score (real bug fix,
  -- 2026-08-19 - see this migration's header) so one matched category
  -- (e.g. 63 real AC-service products) can't fill a company's entire
  -- Signale list on its own.
  delete from signals where type = 'seasonal_push';
  insert into signals (company_id, product_id, type, tier, origin, score, reason, source)
  select company_id, product_id, 'seasonal_push', 1, source_type, score, reason, source
  from (
    select
      d.*,
      row_number() over (partition by d.company_id order by d.score desc, d.product_id) as rn
    from (
      select distinct on (aff.company_id, p.id)
        aff.company_id,
        p.id as product_id,
        aff.source_type,
        case when aff.source_type = 'rule' then 2 else 1.5 end as score,
        ...
        jsonb_build_object('product_id', p.id, 'season', p.season, 'affinity_source', aff.source_type) as source
      from products p
  ...
  -- new_product_match: same shared affinity source, same per-company cap.
  delete from signals where type = 'new_product_match';
  insert into signals (company_id, product_id, type, tier, origin, score, reason, source)
  select company_id, product_id, 'new_product_match', 1, source_type, score, reason, source
  from (
    select
      d.*,
      row_number() over (partition by d.company_id order by d.score desc, d.product_id) as rn
    from (
      select distinct on (aff.company_id, p.id)
        ...
        jsonb_build_object('product_id', p.id, 'launched_at', p.launched_at, 'affinity_source', aff.source_type) as source
      from products p
      join _category_affinity aff on aff.category_code = p.category_code
      join companies c on c.id = aff.company_id and not c.do_not_contact and c.active
