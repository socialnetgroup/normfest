-- Fast-follow from the 2026-07-24 UX pass: Anis asked whether Top-
-- Empfehlungen items should sit until the next VIS refresh or be
-- dismissable once handled. Recommended (and now building) an explicit
-- dismiss action rather than waiting a week -- the flywheel is about
-- fast feedback loops, not stale lists.
--
-- Dismissals must survive fn_refresh_signals()'s full delete+insert per
-- type, so they're a separate table keyed the same way as signals' own
-- dedup index (company, type, product-or-zero-uuid), checked via NOT
-- EXISTS in every insert block below. One-way for v1 (no update/delete
-- policy) -- same immutable-log shape as sales_feedback; if a dismissed
-- signal genuinely needs to come back, that's a service-role cleanup for
-- now, not a self-serve "undo" button.
...
-- Expressions aren't allowed in a table-level `unique(...)` constraint (only
-- plain columns are) -- same reason signals' own dedup key is a unique index,
-- not a constraint.
create unique index idx_signal_dismissals_dedup
  on signal_dismissals (company_id, type, coalesce(product_id, '00000000-0000-0000-0000-000000000000'::uuid));
create index idx_signal_dismissals_company_id on signal_dismissals (company_id);

alter table signal_dismissals enable row level security;

-- Shared visibility, same as signals itself -- once someone on the team
-- handles a recommendation, it should disappear for everyone.
create policy signal_dismissals_select_authenticated
  on signal_dismissals for select
  to authenticated
  using (true);

-- Any agent can dismiss (not admin-only) -- consistent with sales_feedback:
-- the agents acting on these are the ones who know it's been handled.
create policy signal_dismissals_insert_own
  on signal_dismissals for insert
  to authenticated
  with check (dismissed_by = auth.uid());

-- Full fn_refresh_signals() rewrite: same logic as
-- 20260723260000_fn_refresh_signals_enrichment_affinity.sql, plus (a) a
-- NOT EXISTS dismissal check in every insert block and (b) em dashes in
-- reason text replaced with plain hyphens (missed in the tsx-only sweep
-- earlier today -- these strings render directly in the Empfehlungen UI).
