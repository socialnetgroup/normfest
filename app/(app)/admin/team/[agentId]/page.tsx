...
export default async function AgentHistoryPage({ params }: { params: Promise<{ agentId: string }> }) {
  ...
  const [{ data: rows, error }, { data: allRows }, { data: bonusSettings }, { data: snapshotRows }, { data: feedbackRows }] =
    await Promise.all([
      ...
      // Sprechzeit per day (2026-08-18, "s koliko je pricano dodaj u
      // Vollständige Liste Anzeigen") - only ever available for days that
      // have a real dialer_daily_snapshots row (§14 item 24, from
      // 2026-08-10 on), so this stays null on earlier days.
      supabase.from("dialer_daily_snapshots").select("snapshot_date, agents"),
      // Wiedervorlage per day - same "count by creation day" convention
      // already established in /tim (§14 item 97). sales_feedback.agent_id
      // is a PROFILE id, not agents.id (same key-space distinction hit
      // repeatedly elsewhere in this app) - scoped to this agent's own
      // profile_id, not a full-table fetch.
      agent.profile_id
